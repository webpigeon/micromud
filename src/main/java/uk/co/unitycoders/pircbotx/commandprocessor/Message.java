 ...
 * 
 * This allows us to code modules which don't rely on being a channel message or
 * being a private message, but simply treat both the same.
 */
public interface Message {
	 ...
	 * 
	 * This is mostly of use for the parsing function but may be useful to some
	 * commands. Previously, this was the only way to access message contents.
	 * Formatting will have been removed to make processing easier. The trigger
	 * character (if present) will have also been removed.
	 * 
	 ...
	 * 
	 * The string will always be of the form "module action [<args>]". If the
	 * user provided no action the action then action will be set as default.
	 * 
	 ...
	 * 
	 * The tokeniser will parse the message, extract quoted strings and then
	 * split by whitespace. Using this function you can extract arguments from a
	 * message. The zeroth argument is always the command name, the first is
	 * always the subcommand and 2 onwards is always the arguments.
	 * 
	 ...
	 *            the argument ID
	 * @param defaultValue
	 *            a value to return of the argument does not exist
	 ...
	 * 
	 * This will send a response to the user highlighting them. If the message
	 * is a channel message, this will respond in the channel, if this message
	 * was a private message the bot will respond in a private message.
	 * 
	 * @param message
	 *            the message to send
	 */
