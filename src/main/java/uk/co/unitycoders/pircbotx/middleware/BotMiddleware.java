 ...
 * 
 * This allows additional steps for processing commands at two key stages: 1.
 * when the message is first received 2. when the message has been tokenized
 * 
 * At any point the plugin can stop processing of the request by throwing an
 * exception.
 */
public interface BotMiddleware {
	 ...
	 * 
	 * This callback is designed for any processing which should be done before
	 * the rest of the system processes the request (eg. cleaning the input or
	 * changing the command the user entered).
	 * 
	 ...
	 * 
	 * This is processed just before the message is processed by the Module
	 * which will fulfil the request, as a result it is a good place to perform
	 * security checks or inject context into the message object.
	 * 
	 * Middleware is chained together so this method MUST return the text which
	 * should be send to the next piece of middleware.
	 * 
	 * @param processor
	 *            the processor dealing with the request
	 * @param message
